...
int testshortpack() {
  ...
  for (bit = 0; bit < 32; ++bit) {
    ...
    for (length = 0; length <= N; ++length) {
      ...
      if ((bb - (__m128i *)buffer) * sizeof(__m128i) !=
          (unsigned)simdpack_compressedbytes(length, bit)) {
        printf("bug\n");
        return -1;
      }
...
int testlongpack() {
  ...
  for (bit = 0; bit < 32; ++bit) {
    ...
    for (length = 0; length <= N; ++length) {
      ...
      bb = simdpack_length(data, length, (__m128i *)buffer, bit);
      if ((bb - (__m128i *)buffer) * sizeof(__m128i) !=
          (unsigned)simdpack_compressedbytes(length, bit)) {
        printf("bug\n");
        return -1;
...
int testsetd1() {
  ...
  for (bit = 0; bit < 32; ++bit) {
    ...
    data[0] = rand() & ((1U << bit) - 1);
    datazeroes[0] = 0;

    for (i = 1; i < N; ++i) {
      data[i] = data[i - 1] + (rand() & ((1U << bit) - 1));
      datazeroes[i] = 0;
    }
    for (i = 0; i < N; ++i) {
      backdata[i] = 0;
...
int testshortFORpack() {
  ...
  for (bit = 0; bit < 32; ++bit) {
    ...
    for (i = 0; i < N; ++i) {
      data[i] = (rand() & ((1U << bit) - 1)) + offset;
    }
    for (length = 0; length <= N; ++length) {
      for (i = 0; i < N; ++i) {
        backdata[i] = 0;
      }
      rb = simdpackFOR_length(offset, data, length, (__m128i *)buffer, bit);
      if (((rb - (__m128i *)buffer) * sizeof(__m128i)) !=
          (unsigned)simdpackFOR_compressedbytes(length, bit)) {
        return -1;
      }
      simdunpackFOR_length(offset, (__m128i *)buffer, length, backdata, bit);
      for (i = 0; i < length; ++i) {
  ...
  for (gap = 1; gap <= 387420489; gap *= 3) {
    ...
    for (k = 0; k * AVXBlockSize < N; ++k) {
      ...
      */
      ...
      if (avxmaxbits(datain + k * AVXBlockSize) !=
...
int testavx512_2() {
  ...
  for (gap = 1; gap <= 387420489; gap *= 3) {
    ...
    for (k = 0; k * AVX512BlockSize < N; ++k) {
      ...
      /* we read back b1 512-bit vectors at "buffer" and write 512 integers at
       * backbuffer */
      ...
      */
...

      {
        ...
        */
        uint32_t offset = 0;
        /* we compute the bit width */
        const uint32_t b1 = simdmaxbitsd1(offset, datain + k * SIMDBlockSize);
        /* we read 128 integers at "datain + k * SIMDBlockSize" and
           write b1 128-bit vectors at "buffer" */
        simdpackwithoutmaskd1(offset, datain + k * SIMDBlockSize, buffer, b1);
        /* we read back b1 128-bit vectors at "buffer" and write 128 integers at
         * backbuffer */
...
int test_simdpackedsearch() {
  ...
  for (b = 1; b <= 32; b++) {
    ...
    for (i = 1; i <= 128; i++) {
      initial = _mm_set1_epi32(init);
      assert(simdsearchd1(&initial, (__m128i *)out, b, (uint32_t)i, &result) ==
             i - 1);
      assert(result == (unsigned)i);
    }
    initial = _mm_set1_epi32(init);
    assert(simdsearchd1(&initial, (__m128i *)out, b, 200, &result) == 128);
    assert(result > 200);
  }
...
int test_simdpackedsearchFOR() {
  ...
  for (b = 1; b <= 32; b++) {
    ...
    for (i = 0; i < 128; i++) {
      int x = simdsearchwithlengthFOR(tmin, (__m128i *)out, tb, 128, buffer[i],
                                      &result);
...
int test_simdpackedsearch_advanced() {
  ...
  for (b = 0; b <= 32; b++) {
    ...
    for (i = 0; i < 128; i++) {
      int pos;
      initial = _mm_set1_epi32(init);
      pos = simdsearchd1(&initial, (__m128i *)out, b, buffer[i], &result);
    ...
    for (i = 0; i < 128; i++) {
      ...
      if (buffer[i] == 0)
        continue;
      initial = _mm_set1_epi32(init);
      pos = simdsearchd1(&initial, (__m128i *)out, b, buffer[i] - 1, &result);
      assert(pos == simdsearchwithlengthd1(init, (__m128i *)out, b, 128,
                                           buffer[i] - 1, &result));
...
int test_simdpackedselect_advanced() {
  ...
  for (b = 0; b <= 32; b++) {
    ...
    /* now perform the searches */
    for (i = 0; i < 128; i++) {
      uint32_t valretrieved =
          simdselectd1(initial, (__m128i *)out, b, (uint32_t)i);
...
int main() {
  ...
  r = issue21();
  if (r) {
    printf("test failure issue21\n");
    return r;
  }
  r = issue21FOR();
  if (r) {
    printf("test failure issue21FOR\n");
    return r;
  }
#ifdef __AVX512F__
  r = testbabyavx512();
